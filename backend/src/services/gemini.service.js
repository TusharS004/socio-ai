...
export const generateWithGemini = async (mediaPath, content) => {
    ...
    await processMediaPaths(mediaPath, video_model, image_model, fileManager);
...
async function processMediaPaths(mediaPath, video_model, image_model, fileManager) {
    const processed = [];
    for (const { url, publicId } of mediaPath) {
        try {
            if (url.includes(".mp4")) {
                const result = await processVideo(url, publicId, video_model, fileManager);
                processed.push({ type: 'video', publicId, result });
            } else if (url.includes(".jpg") || url.includes(".png") || url.includes(".jpeg")) {
                const result = await processImage(url, publicId, image_model, fileManager);
                processed.push({ type: 'image', publicId, result });
            }
        } catch (error) {
            await fs.appendFile("file_generation_log.txt", 
  ...
  async function processVideo(url, public_id, video_model, fileManager) {
  ...
  async function processImage(url, public_id, image_model, fileManager) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          await fs.appendFile(outputFilePath, `another listing\n\n${content}` + "\n\n\n", "utf8");
      ...
      console.error("Error reading directory or writing to file:", error);
    }
}
